package com.jungel.coinoffline.eos.eos4j.ecc;

import com.jungel.coinoffline.eos.eos4j.api.vo.transaction.push.TxSign;
import com.jungel.coinoffline.eos.eos4j.utils.Base58;
import com.jungel.coinoffline.eos.eos4j.utils.ByteBuffer;
import com.jungel.coinoffline.eos.eos4j.utils.ByteUtils;
import com.jungel.coinoffline.eos.eos4j.utils.EException;
import com.jungel.coinoffline.eos.eos4j.utils.Hex;
import com.jungel.coinoffline.eos.eos4j.utils.ObjectUtils;
import com.jungel.coinoffline.eos.eos4j.utils.Ripemd160;
import com.jungel.coinoffline.eos.eos4j.utils.Sha;
...
public class EccTool {
    ...
    private static BigInteger privateKey(String pk) {
        ...
        if (private_wif[0] != version) {
            throw new EException("version_error", "Expected version " + 0x80 + ", instead got " +
                    version);
        }
    ...
    public static String  signHash(String pk, byte[] b) {
        ...
        System.out.print("TEST-- checksum : ");
        for (byte c : checksum) {
            System.out.print(c);
        }
        System.out.println("");
        byte[] signatureString = ByteUtils.concat(pub_buf, ByteUtils.copy(checksum, 0, 4));
        System.out.print("TEST-- signatureString : ");
        for (byte c : signatureString) {
            System.out.print(c);
        }
        System.out.println("");
     ...
     * @return
     */
    public static String signTransaction(String privateKey, TxSign push, int type) {
        // tx
        ByteBuffer bf = new ByteBuffer();
        if (type == TYPE_ACCOUNT) {
            ObjectUtils.writeAccountBytes(push, bf);
        } else if (type == TYPE_TRANSFER) {
            ObjectUtils.writeTransferBytes(push, bf);
        } else {
            ObjectUtils.writeBytes(push, bf);
        }
        byte[] real = bf.getBuffer();
        System.out.print("TEST-- real1 : ");
        for (byte b : real) {
            System.out.print(b);
        }
        System.out.println("");
